...
m = Map("profile", translate("Modem Connection Profiles "),translate(""))									 
...
if profsave == "1" then
	...
	ds = m:section(TypedSection, "simpin", translate("SIM card Pin code:"), translate("If there is no SIM PIN code in the configuration file, it will be used"))
	ds.anonymous = true
...
di = m:section(TypedSection, "default", translate("Default"), translate("Generally, you only need to fill in the APN of your operator. Generally, it has been automatically loaded and identified. You can also continue to fill in Tips: The module will take effect after the next reconnection. If you want to take effect now, please save and restart the module) ## Huawei Skyline 5G SA APN: 5gscuiot ##"))
di.anonymous = true
...
ynl = di:taboption(this_tab, ListValue, "hostless", translate("TTL Settings"));
ynl:value("0", "NO")
...
mtz = di:taboption(this_tab, ListValue, "tzone", translate("Auto Set Timezone"), translate("Tips: The module will automatically set the time zone after connecting to the network"));
mtz:value("0", "No")
mtz:value("1", translate("Yes"))
mtz.default=1

ml = di:taboption(this_tab, ListValue, "lock", translate("Lock to Provider :"));
ml:value("0", translate("NOT"))
...
mdns1 = di:taboption(this_taba, Value, "dns1", translate("Custom DNS Server 1 :")); 
mdns1.rmempty = true;
mdns1.optional=false;
mdns1.datatype = "ipaddr"

mdns2 = di:taboption(this_taba, Value, "dns2", translate("Custom DNS Server 2 :")); 
mdns2.rmempty = true;
mdns2.optional=false;
mdns2.datatype = "ipaddr"

mdns3 = di:taboption(this_taba, Value, "dns3", translate("Custom DNS Server 3 :")); 
mdns3.rmempty = true;
mdns3.optional=false;
mdns3.datatype = "ipaddr"

mdns4 = di:taboption(this_taba, Value, "dns4", translate("Custom DNS Server 4 :")); 
mdns4.rmempty = true;
...
if nixio.fs.access("/etc/config/mwan3") then
	mlb = di:taboption(this_taba, ListValue, "lb", translate("Enable traffic load balancing in multiple 5G modules :"));
...
mtu = di:taboption(this_taba, Value, "mtu", translate("Custom MTU :"),
		translate("It is not recommended to mess around, you can only customize the value in the range of 1420 to 150"));
mtu.optional=true
mtu.rmempty = true
mtu.default = "1500"
...
alive = di:taboption(this_tab, ListValue, "alive", translate("Connection Monitoring :")); 
alive.rmempty = true;
alive:value("0", translate("Disable"))
alive:value("1", translate("Enable dropped writing to the system log"))
alive:value("2", translate("Enable 5G module disconnection, then the route will restart automatically"))
alive:value("3", translate("Enable 5G module disconnection then automatic redial"))
alive:value("4", translate("Enable the 5G module to drop the power on module, and the routing GPIO support is required, otherwise other methods will be used to reconnect"))
alive.default=0

reliability = di:taboption(this_tab, Value, "reliability", translate("Tracking reliability  :"),
		translate("Range: 1-100, these IP addresses must be pinged for this communication link to be considered networked"))
...
interval:value("300", translate("5 minute"))
interval:value("600", translate("10 minute"))
interval:value("900", translate("15 minute"))
interval:value("1800", translate("30 minute"))
...
timeout.default = "2"
timeout:value("1", translate("1 seconds"))
...
if (multilock == "0") or (multilock == "1" and rootlock == "1") then
	...
	phone = di:taboption(this_tab, Value, "phone", translate("Phone Number :"), translate("12223334444"))
	phone.default = "Phone Number to send Text Message"
...
imei = s:taboption(this_ctab, Value, "imei", translate("Modem IMEI :")); 
imei.optional=false;
imei:depends("select", "1")
imei.datatype = "uinteger"
imei.default="1234567"

model = s:taboption(this_ctab, Value, "model", translate("Model Name :")); 
model.optional=false;
model:depends("select", "2")
model.default="xxxx"

imsi = s:taboption(this_ctab, Value, "imsi", translate("SIM IMSI  :")); 
imsi.optional=false;
imsi:depends("select", "3")
imsi.datatype = "uinteger"
imsi.default="1234567"

iccid = s:taboption(this_ctab, Value, "iccid", translate("SIM ICCID :")); 
...
imei1 = s:taboption(this_ctab, Value, "imei1", translate("Modem IMEI :")); 
imei1.optional=false;
imei1:depends("select1", "1")
imei1.datatype = "uinteger"
imei1.default="1234567"

model1 = s:taboption(this_ctab, Value, "model1", translate("Model Name :")); 
model1.optional=false;
model1:depends("select1", "2")
model1.default="xxxx"

imsi1 = s:taboption(this_ctab, Value, "imsi1", translate("SIM IMSI :")); 
imsi1.optional=false;
imsi1:depends("select1", "3")
imsi1.datatype = "uinteger"
imsi1.default="1234567"

iccid1 = s:taboption(this_ctab, Value, "iccid1", translate("SIM ICCID :")); 
...
if nixio.fs.access("/etc/config/mwan3") then
	cmlb = s:taboption(this_ctaba, ListValue, "lb", translate("Enable traffic load balancing in multiple 5G modules :"));
...
calive = s:taboption(this_ctab, ListValue, "alive", translate("Connection Monitoring :")); 
calive.rmempty = true;
calive:value("0", translate("Disable"))
calive:value("1", translate("Enable dropped writing to the system log"))
calive:value("2", translate("Enable 5G module disconnection, then the route will restart automatically"))
calive:value("3", translate("Enable 5G module disconnection then automatic redial"))
calive:value("4", translate("Enable the 5G module to drop the power on module, and the routing GPIO support is required, otherwise other methods will be used to reconnect"))
calive.default=0
...
if (multilock == "0") or (multilock == "1" and rootlock == "1") then
	...
	phone = s:taboption(this_ctab, Value, "phone", translate("Phone Number  :"), translate("12223334444"))
